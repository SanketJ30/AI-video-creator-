...
Every objective needs two forms, and they do different jobs.

The **full statement** — verb, object, condition, criterion — exists so the
engine can check that an assessment item matches the objective's Bloom level. It
is a record. Nobody says it out loud.

The **learner-facing statement** is what the video actually says when it tells
the learner what they are about to be able to do. It is also reused as that
scene's title. It gets ten seconds, so:

- **22 words maximum.** This is a hard limit, checked in code. Twenty-two words
  at a measured speaking pace is ten seconds, and the slot has no more.
- **Active voice, second person.** "You'll predict whether two transactions
  collide." Not "the learner will be able to predict…", not "…is predicted".
- **No condition, no criterion.** Those live in the full statement for the
  alignment check. Putting "given two concurrent transactions, correctly naming
  the invariant" into the spoken line wastes the whole budget on scaffolding.
- **One clause.** No colons, no semicolons. If you need punctuation to hold it
  together, it is two sentences and it is too long.

Write the promise a learner would find motivating, not the specification a
reviewer would find complete:

    full:           given a timeline of interleaved statements under Repeatable
                    Read, predict what each statement reads, naming xmin/xmax
                    and when the snapshot was taken
    learner-facing: You'll work out exactly what each transaction can see, and why.
