...
## What the six designed templates expect

The renderer implements a design system. Six templates have a designed layout
and their slots exist to serve it:

**`cold_open`** — the question or paradox the lesson resolves, held large.
`headline` is that question, not a summary and not the answer. `module_label`
is a short orienting label above it (`MODULE 03 · WRITE SKEW`). `premise_line`
is one supporting line beneath, appearing after the question is readable.

**`title_card`** — top-left anchored orientation. `title`, `subtitle`, and an
optional `module_label`. Not a hook; this is the learner finding their place.

**`key_phrase`** — one statement made memorable. `phrase` is the statement;
`emphasis` is the clause it RESOLVES TO, and the renderer colours a cued
emphasis with the answer role rather than the attention role. So put the
resolution there, not a repeat of the phrase.

**`state_timeline`** — parallel or sequential progression. Every step names its
`track`, and steps sit on a SHARED time axis so step 3 in one lane is visibly
after step 2 in the other. Keep step labels to a few words: they are drawn
under a marker, not in a paragraph.

**`table_build`** — a comparison built row by row. `columns` are short header
words; each row's `cells` fill every column in order. `highlight_row` is the
row the scene RESOLVES to, drawn in the answer role at the end.

**`concept_illustration`** — typographic and diagrammatic, no imagery required.
Use `steps` for a flow of 3–5 short labelled blocks (`RAW DATA`, `FEATURES`,
`MODEL`) with `caption` as the heading above them. Use `caption` alone for a
single line. Do not write a paragraph into a block label.

The remaining templates render legibly but have no designed layout; prefer one
of the six when either would serve.

## How much text fits

The content region is 1728 × 782 px and type is never shrunk to make copy fit —
a scene whose copy overflows is REJECTED and returned to you, not made smaller.
So write to the slot: headlines are one sentence, block labels are two or three
words, table cells are short phrases rather than clauses.

## What state the scene is showing

Every scene carries a `resolution_state`. It is a property of the WHOLE scene —
what the learner is being shown — not an event at a moment. Cues say *where to
look*; this says *what kind of thing they are looking at*.

Four values, and the distinction between them is the point:

- **`neutral`** — explanation in progress. No claim about correctness. Most
  scenes are this: a hook, a definition, an objective, a walkthrough of a
  mechanism.
- **`broken`** — the scene shows the invariant violated. The failure itself: a
  rule that no longer holds, a check that passed when it should not have, an
  end state that is wrong. This is what the lesson is diagnosing.
- **`caution`** — the scene shows a cost, a precondition or a caveat. Something
  that works but has a price, or that only works if something else is true.
- **`resolved`** — the scene shows the remedy that works, or the correct final
  state. The answer the lesson arrives at.

### Deciding from the narration and the slot

The Gagné slot is strong evidence and the narration settles it:

- a `feedback` slot revealing the correct answer is **`resolved`**
- a `present` slot demonstrating the anomaly is **`broken`**
- a `guide` slot weighing options where one costs something is **`caution`**
- `hook`, `objective`, `recall`, `elicit`, `assess`, `retain` are usually
  **`neutral`** — they set up, ask, or review rather than assert a state

Do not mark a scene `resolved` because it ends well, or `broken` because the
subject is a failure mode. Mark what the scene ITSELF shows. A scene explaining
why an anomaly happens is `neutral` — it is explaining, not exhibiting. The
scene where the table reads zero doctors on call is `broken`.

Use `neutral` when unsure. A wrong state claim is worse than no state claim: it
tells the learner something is the answer when it is not.
